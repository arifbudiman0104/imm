            ...
            <div class="flex flex-col justify-between mb-5 md:flex-row">
                ...
                <div class="flex items-center">
                    ...
                    <p x-data="{ show: true }" x-show="show" x-transition x-init="setTimeout(() => show = false, 5000)"
                        class="text-sm text-green-600 dark:text-green-400">{{ __('Successfuly change user role to
                        Admin.') }}</p>
                    @endif
                    ...
                    <p x-data="{ show: true }" x-show="show" x-transition x-init="setTimeout(() => show = false, 5000)"
                        class="text-sm text-green-600 dark:text-green-400">{{ __('Successfuly verified user.') }}
                    </p>
                    @endif

                    @if (session('status') === 'user-not-verified')
                    <p x-data="{ show: true }" x-show="show" x-transition x-init="setTimeout(() => show = false, 5000)"
                        class="text-sm text-red-600 dark:text-red-400">{{ __("Failed verified user, Make sure user
                        profile is complete.") }}</p>
                    @endif

                    @if (session('status') === 'user-unverified')
                    <p x-data="{ show: true }" x-show="show" x-transition x-init="setTimeout(() => show = false, 5000)"
                        class="text-sm text-green-600 dark:text-green-400">{{ __('Successfuly unverified user.') }}
                    </p>
                    @endif

                    @if (session('status') === 'user-deleted')
                    <p x-data="{ show: true }" x-show="show" x-transition x-init="setTimeout(() => show = false, 5000)"
                        class="text-sm text-green-600 dark:text-green-400">{{ __('Successfuly delete user.') }}
                    </p>
                    @endif
                </div>
